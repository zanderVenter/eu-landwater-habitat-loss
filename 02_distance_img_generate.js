/**
 * 02 - Generate distance-to-coast and distance-to-freshwater images
 *
 * Pipeline step 2 of 4 (Google Earth Engine, run manually in the Code
 * Editor). See README.md for the full pipeline order.
 *
 * Re-derives its own coastal/riparian strata at 30m (rather than reusing
 * the 50m strataImg from 01_strata_img_generate.js) because the coarser
 * resolution there causes irregularities in the distance transform.
 *
 * What this does:
 *   1. Rasterizes the coastline (from the Copernicus coastal zone polygons)
 *      and the freshwater portion of the riparian zone polygons
 *      (CODE_1_18 == 8, i.e. the actual river/lake pixels within the
 *      riparian buffer, not the whole buffer).
 *   2. Computes a Euclidean distance transform from each, in metres,
 *      capped at 10km (matches the max buffer distance analysed in the
 *      manuscript) and masked to land.
 *   3. Exports one 2-band (distCoast, distWater) image per EEA-39 grid
 *      cell to an Earth Engine Image Collection asset.
 *
 * Inputs (Earth Engine assets, must already exist):
 *   - projects/nina/Europe_misc/CLMS_CLCplus_RASTER_2018_010m_eu_03035_V1_1
 *   - projects/nina/Europe_misc/CLMS_euhydro_angerman_coastal_p_v013 / _transit_p_v013
 *   - projects/nina/Europe_misc/CLMS_riparian_zone/rpz_DU001A_2018 .. DU043A_2018
 *   - projects/nina/Arena/export_grid_EEA39
 *
 * Output: Image assets at
 *   projects/gee-zander-nina/assets/Arena/distImg_riparian_coastal/ID_<CellCode>
 * Band 0 = distCoast, band 1 = distWater (freshwater). Consumed by
 * 03_areas_extract_grid.js and 04_areas_extract_countries.js.
 *
...
// Freshwater = the water pixels within the riparian buffer polygons
// (CODE_1_18 == 8), i.e. the actual river/lake surface, not the full buffer.
